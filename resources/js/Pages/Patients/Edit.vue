<template>
  <AppLayout :title="$t('patients.edit')">
    <div class="max-w-2xl mx-auto">
      <div class="mb-6">
        <Link :href="`/patients/${patient.uuid}`" class="text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors flex items-center gap-1">
      ...
      <BaseCard>
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="pt-4 flex justify-end gap-3 rtl:space-x-reverse border-t border-slate-100 dark:border-slate-700 mt-6">
            <Link :href="`/patients/${patient.uuid}`" class="px-5 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors rounded-lg">
              {{ $t('common.cancel') }}
            </Link>
            <BaseButton type="submit" :loading="form.processing">
              {{ $t('patients.update_patient') }}
            </BaseButton>
          </div>
        </form>
      </BaseCard>
