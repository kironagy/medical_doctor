<template>
  <WorkspaceModal :modelValue="modelValue" @update:modelValue="$emit('update:modelValue', $event)" @close="$emit('update:modelValue', false)" :title="$t('workspace.category_manager.title')" size="lg">
    <div class="space-y-4">
      <div class="space-y-3">
        <div v-for="(cat, idx) in categories" :key="cat.slug"
          class="flex flex-col gap-3 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800/50"
        >
          <!-- Top Row: Order, Name, Eye, Trash -->
          <div class="flex items-center gap-2">
            <div class="flex flex-col gap-0.5 text-slate-400 shrink-0">
            ...
            <input v-model="cat.name" class="flex-1 input-field text-sm py-1 px-2" :placeholder="$t('workspace.category_manager.name_placeholder')" />
            
            <button type="button" class="p-1.5 rounded-lg transition-colors shrink-0"
            ...
            <button type="button" class="p-1.5 rounded-lg text-rose-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors shrink-0" @click="removeCategory(idx)">
      ...
      <div v-if="showAddForm" class="flex items-center gap-2 p-3 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-800/30">
        <input ref="newNameInput" v-model="newCatName" class="flex-1 input-field text-sm py-1 px-2" :placeholder="$t('workspace.category_manager.new_name_placeholder')"
          @keydown.enter.prevent="addCategory" />
        <BaseButton size="sm" variant="secondary" @click="addCategory">{{ $t('workspace.category_manager.add') }}</BaseButton>
        <BaseButton size="sm" variant="ghost" @click="cancelAdd">{{ $t('workspace.category_manager.cancel') }}</BaseButton>
      </div>

      <div class="flex items-center gap-3 pt-2">
        <BaseButton v-if="!showAddForm" variant="secondary" size="sm" @click="openAddForm">
          <PlusIcon class="w-4 h-4 me-1" /> {{ $t('workspace.category_manager.add_category') }}
        </BaseButton>
        <div class="flex-1" />
        <BaseButton :loading="saving" @click="save">{{ $t('workspace.category_manager.save_changes') }}</BaseButton>
      </div>
    </div>
  </WorkspaceModal>
</template>
