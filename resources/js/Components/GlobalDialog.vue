  ...
  >
    <div class="flex items-start gap-4">
      ...
      >
        ...
        <svg v-else-if="state.style === 'success'" class="w-6 h-6" :class="iconStyles.text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
        </svg>
        <svg v-else-if="state.style === 'warning'" class="w-6 h-6" :class="iconStyles.text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <svg v-else class="w-6 h-6" :class="iconStyles.text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      </div>
      <div class="flex-1 mt-1">
        <h3 class="text-lg font-heading font-bold text-slate-900 dark:text-white">
          {{ state.title }}
        </h3>
