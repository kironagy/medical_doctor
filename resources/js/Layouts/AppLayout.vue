      ...
      <div class="p-4 mt-auto mb-2 mx-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm" dir="rtl">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold text-slate-900 dark:text-white truncate">{{ $page.props.auth?.user?.name || 'أحمد سيد' }}</p>
            <p class="text-xs text-slate-500 dark:text-slate-400 truncate">{{ $page.props.auth?.user?.email || '' }}</p>
          </div>
        </div>
        
        <div class="space-y-1 border-t border-slate-100 dark:border-slate-800 pt-2 mb-2">
          <Link href="/profile" class="flex items-center gap-3 w-full text-right text-sm text-slate-700 dark:text-slate-300 font-medium hover:text-primary-600 dark:hover:text-primary-400 p-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">
          ...
          <Link href="/support" class="flex items-center gap-3 w-full text-right text-sm text-slate-700 dark:text-slate-300 font-medium hover:text-primary-600 dark:hover:text-primary-400 p-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">
            <svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            الدعم الفني
          </Link>
        </div>
