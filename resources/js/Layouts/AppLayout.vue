<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row">
    ...
    <div v-if="mobileMenuOpen" class="fixed inset-0 bg-slate-900/50 z-40 md:hidden" @click="mobileMenuOpen = false"></div>
    
    <!-- Mobile Drawer -->
    <aside class="fixed inset-y-0 left-0 rtl:left-auto rtl:right-0 w-64 bg-white dark:bg-slate-900 shadow-xl z-50 transform transition-transform duration-300 md:hidden flex flex-col" :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'">
