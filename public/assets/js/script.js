...
const translations = {
    ...
    en: {
        ...
        delete: "Delete",
        deleteConfirm:
            "Are you sure you want to delete this patient and all their files?",
...
function apiUrl(path) {
    ...
    try {
        ...
        if (baseUrl.pathname && normalizedPath.startsWith(baseUrl.pathname))
...
function initApp() {
    ...
    toggleTheme();
    toggleTheme(); // Quick way to init icons
}
...
function updateDashboardStats() {
    ...
    if (storageUsed) {
        const mockStorageMb = ((dashboardStats.totalFiles || 0) * 2.4).toFixed(
            1,
        );
...
async function handleAddPatient(e) {
    ...
    const diagnosis = document.getElementById("patientDiagnosis")
...
function switchFileCategory(cat) {
    ...
    document
        .querySelectorAll(".file-categories-sidebar .cat-btn")
        .forEach((btn) => {
            btn.classList.remove("active");
            if (
                (cat === "all" &&
                    btn.textContent.includes("الكل") &&
                    !btn.textContent.includes("أدوية")) ||
    ...
    if (titleEl) {
        if (cat === "all") titleEl.textContent = "جميع الملفات";
            ...
            const activeBtn = document.querySelector(
                ".file-categories-sidebar .cat-btn.active",
            );
            titleEl.textContent = activeBtn
...
function handleFileFilter() {
    const query = document.getElementById("fileSearchInput")
        ? document.getElementById("fileSearchInput").value.toLowerCase().trim()
        : "";
...
function renderPatientFiles(patientId) {
    ...
    let patientFiles = filesData
        .filter((f) => f.patient_id === patientId || f.patientId === patientId)
        .sort((a, b) => b.id - a.id);

    if (currentFileFilter.category !== "all") {
        patientFiles = patientFiles.filter(
            (f) => f.category === currentFileFilter.category,
        );
    }

    if (currentFileFilter.type !== "all") {
        patientFiles = patientFiles.filter((f) =>
            f.type.includes(currentFileFilter.type),
        );
    }
    ...
    if (currentFileFilter.query) {
        patientFiles = patientFiles.filter(
            (f) =>
                f.title.toLowerCase().includes(currentFileFilter.query) ||
                (f.desc &&
                    f.desc.toLowerCase().includes(currentFileFilter.query)),
        );
    }

    if (currentFileFilter.from) {
        patientFiles = patientFiles.filter(
            (f) => f.date >= currentFileFilter.from,
        );
    }

    if (currentFileFilter.to) {
        patientFiles = patientFiles.filter(
            (f) => f.date <= currentFileFilter.to,
        );
    }
    ...
    patientFiles.forEach((file) => {
        ...
        card.innerHTML = `
            ...
            <div class="card-actions" style="margin-top: 0; padding-top: 1.25rem; display: flex; gap: 0.5rem;">
                ...
                ${
                    fileSrc
                        ? `<a href="${fileSrc}" download="${displayName}" class="btn btn-outline" style="width: 45px; height: 45px; justify-content: center; padding: 0; flex-shrink: 0;" title="تحميل">
...
function closeUploadModal() {
    ...
    if (nameEl) {
        nameEl.textContent = "لم يُختر ملف بعد";
        nameEl.style.color = "var(--text-muted)";
    }
    const displayEl = document.getElementById("fileNameDisplay");
    if (displayEl) {
        const icon = displayEl.querySelector("i");
        if (icon) {
...
function handleFileSelect(event) {
    ...
    if (file) {
        ...
        if (nameEl) {
            ...
            if (displayEl) {
                displayEl.querySelector("i").className =
...
function handleUploadFile(e) {
    ...
    const category = document.getElementById("fileCategory")
...
function viewFile(id) {
    ...
    if (file.type === "image") {
        ...
        const src =
            file.file_path ||
            file.data ||
            `https://placehold.co/800x600/0F172A/FFFFFF.png?text=Image`;
...
async function deleteFile(fileId, patientId) {
    ...
    try {
        const res = await fetch(
            apiUrl(`/patients/${patientId}/files/${fileId}`),
            {
                method: "DELETE",
                headers: apiHeaders({ Accept: "application/json" }),
            },
...
function openEditModal(id) {
    ...
    if (p) {
        ...
        if (document.getElementById("editPatientDiagnosis"))
...
async function handleEditPatient(e) {
    ...
    const diagnosis = document.getElementById("editPatientDiagnosis")
